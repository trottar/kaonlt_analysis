...
#include <TProfile.h>
#include <TObjArray.h>
void plot_coll_fp(TString basename = "") {
   if (basename=="") {
...
TString inputroot= basename+".root";
//TString inputroot = "kpp_shms_488.root";
...
TH2F *hyfpVSdel = new TH2F("hyfpVSdel",";yfp (cm);delta (%) ",100,-40.,40.,100,-20.,20.);
 ...
 int numxsh,numysh;
   ...
   //
TCanvas *ctar = new TCanvas("ctar","sieve",1024,768);
ctar->Divide(2,2);
ctar->cd(1);
...
ctar->cd(4);
 ...
 //
   //
TCanvas *cfpall = new TCanvas("cfpall","focal plane for all events",1024,768);
cfpall->Divide(2,2);
cfpall->SetLogz(1);
cfpall->cd(1);
 gPad->SetLogz(1);
...
TCanvas *cfp = new TCanvas("cfp","focal plane",1024,768);
cfp->Divide(2,3);
cfp->cd(1);
 int ifirst=1;
 for ( int iy = 0; iy < numy ; iy++) {
