...
TString q2name[nftot]={"ideal","_m0005","_m001","_m002","_m006"};
 ...
 //
  TString inputroot;
   ...
   for (Int_t nc=0;nc<4;nc++) {
     dcomp[nc] =  new TCanvas(Form("dcomp_%d",nc),xlabel,1024,768);
     dcomp[nc]->Divide(1,1);
     dcomp[nc]->cd(1);
 ...
 if (nf!=0)fhistdiff[nf][nc]->Draw("same");
   ...
   ccomp[nc] = new TCanvas(Form("ccomp_%d",nc),xlabel,1024,768);
    ccomp[nc]->Divide(2,3);
   for (nf=0;nf<nftot;nf++) {
     ccomp[nc]->cd(nf+1);
   ...
   for (Int_t nc=0;nc<4;nc++) {
   TString xlabel= label1 + ht[nc] + label2;
   rmscomp[nc] = new TCanvas(Form("rmscomp_%d",nc),xlabel,1024,768);
   ...
   TLegend *sigmaleg[4];
   for (Int_t nc=0;nc<4;nc++) {
     sigmacomp[nc] = new TCanvas(Form("sigmacomp_%d",nc),xlabel,1024,768);
 ...
 if (nf!=0)fhistsigma[nf][nc]->Draw("same");
   ...
   TLegend *meanleg[4];
   for (Int_t nc=0;nc<4;nc++) {
     meancomp[nc] = new TCanvas(Form("meancomp_%d",nc),xlabel,1024,768);
